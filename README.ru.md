...
1. Лучший способ, с помощью функции
    * `Registrino( defineFunction )`
        * `defineFunction( r )` - это функция, которая будет сразу же вызвана с параметром
            * `r` - будет уже хранить новый и пустой объект **Регистра**
            * Функция должна вернуть объект, содержащий ново-созданные **Функции** и **Переменные Регистра** для открытого доступа.
...
* `Registrino.createVariable( value )`, и
